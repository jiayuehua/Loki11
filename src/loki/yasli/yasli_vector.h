...
private:
  ...
  void init(non_iterator n, non_iterator datum, priority_tag<false>, const Allocator &a)
  {
...
public:
  ...
  {
    init(first, last, priority_tag < yasli_nstd::is_class<InputIterator>::value || yasli_nstd::is_pointer<InputIterator>::value > (), a);
  }
...
public:
  ...
  void assign(InputIterator first, InputIterator last)
  {
    ...
    assign_pre_impl(first, last, priority_tag < yasli_nstd::is_class<InputIterator>::value || yasli_nstd::is_pointer<InputIterator>::value > ());
  }
...
private://-------ASSIGN IMPLEMENTATION
  ...
  template<class InputIterator>
  void assign_pre_impl(InputIterator n, InputIterator datum, priority_tag<false>)
  {
...
public:
  ...
  {
    insert_pre_impl(position, first, last, priority_tag < yasli_nstd::is_class<InputIterator>::value || yasli_nstd::is_pointer<InputIterator>::value > ());
  }

private:
  template<class InputIterator, class looks_like_iterator>
  void
    insert_pre_impl(iterator position, InputIterator first, InputIterator last, looks_like_iterator)
  {
    insert_impl(position, first, last, typename std::iterator_traits<InputIterator>::iterator_category());
  }

  template<class non_iterator>
  void insert_pre_impl(iterator position, non_iterator n, non_iterator x, priority_tag<false>)
...
struct move_traits<yasli::vector<T, A>> : public yasli_nstd::type_selector<sizeof(yasli::vector<T, A>) != (3 * sizeof(T *)), memmove_traits<std::complex<T>>, safe_move_traits<std::complex<T>>>::result
{
};
}// namespace yasli_protocols
