...
template<typename T>
using sizeof_type = mp::mp_size_t<sizeof(T)>;
...
private:
  ...
  typedef mp::mp_front<TList> Head;
  typedef mp::mp_rest<TList> Tail;
...
template<>
union ConfigurableUnion<mp::mp_list<>> {
};
...
struct MyMaxSize
{
  //typedef mp::mp_list<int, double, char> list;
  using r = mp::mp_transform<sizeof_type, TList>;

public:
  using type = mp::mp_max_element<r, mp::mp_less>;
};
template<class TList>
using MyMaxSize_t = typename MyMaxSize<TList>::type;
//template<class TList>
//static constexpr auto MyMaxSize_v = MyMaxSize_t::value;
...
{
  enum { maxSize = MyMaxSize_t<TList>::value };

  template<std::size_t Size>
  struct LessEqual
  {
    template<class T>
    using Cmp = mp::mp_bool<sizeof(T) <= Size>;
  };
  typedef mp::mp_list<char, short, int, float, double> list;
  using AlignTypes = mp::mp_filter<LessEqual<maxSize>::template Cmp, TypesOfAllAlignments>;
...
{
public:
  typedef mp::mp_transform<std::add_const, TList> Result;
};
...
struct Converter
{
  ...
  {
  private:
    void DoVisit(const Type &obj, priority_tag<true>)
    {
      new (this->storageForDestination) VariantTo(obj);
    }
    void DoVisit(const Type &, priority_tag<false>)
    {
      std::cout << "error1\n"
                << std::endl;
    ...
    virtual void Visit(const Type &obj)
    {
      ...
      enum { dispatch = mp::mp_find<TList, Type>::value != mp::mp_size<TList>::value };
...
struct ConverterTo
{
  ...
  private:
    ...
    {
      using namespace Loki;
      enum { dispatch = std::is_convertible_v<Type, T> };
      this->DoVisit(obj, Int2Type<dispatch>());
    }
  };
...
class Variant;
template<class... TA, class AlignedPODType>
class Variant<mp::mp_list<TA...>, AlignedPODType>
{
  // VC7: fatal error C1067: compiler limit :
  // debug information module size exceeded
...
public:
  typedef mp::mp_list<TA...> Types;
  typedef mp::mp_list<TA...> TList;
...
private:
  ...
  {
    static_assert((mp::mp_find<TList, T>::value >= 0), " Invalid_Type_Used_As_Initializer");
...
public:
  ...
  typedef ::Loki::Visitor<void, false, TA...>
    StrictVisitor;
  typedef ::Loki::Visitor<void, false, std::add_const_t<TA>...>
    ConstStrictVisitor;
...
private:
  ...
  private:
    ...
    static void Accept(VariantType &var, StrictVisitor &visitor)
    {
      ...
      ::Loki::VisitorProductBase<T, RType> &v = visitor;
    ...
    static void AcceptConst(const VariantType &var, ConstStrictVisitor &visitor)
    {
      ...
      ::Loki::VisitorProductBase<const T, RType> &v = visitor;
  ...
  template<class T>
  friend struct VTableImpl;

private:// should be private; some compilers prefer 'public' :o}
  enum { neededSize = MyMaxSize_t<TList>::value };
