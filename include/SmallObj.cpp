...
class Chunk
{
 private:
  ...
  inline bool IsFilled(void) const { return (0 == blocksAvailable_); }
...
class FixedAllocator
{
 ...
 public:
  ...
  inline Chunk* HasBlock(void* p) { return const_cast<Chunk*>(const_cast<const FixedAllocator*>(this)->HasBlock(p)); }
};
...
void Chunk::Deallocate(void* p, std::size_t blockSize)
{
  ...
  if (0 < blocksAvailable_) assert(firstAvailableBlock_ != index);
#endif
...
bool Chunk::IsBlockAvailable(void* p, unsigned char numBlocks, std::size_t blockSize) const
{
  ...
  if (index == blockIndex) return true;
...
FixedAllocator::FixedAllocator()
    : blockSize_(0), numBlocks_(0), chunks_(0), allocChunk_(NULL), deallocChunk_(NULL), emptyChunk_(NULL)
{
}
...
#endif
  for (ChunkIter i(chunks_.begin()); i != chunks_.end(); ++i) i->Release();
}
...
bool FixedAllocator::IsCorrupt(void) const
{
  ...
  if (isEmpty) {
    ...
    for (ChunkCIter it(start); it != last; ++it) {
      const Chunk& chunk = *it;
      if (chunk.IsCorrupt(numBlocks_, blockSize_, true)) return true;
    }
  }
...
const Chunk* FixedAllocator::HasBlock(void* p) const
{
  ...
  for (ChunkCIter it(chunks_.begin()); it != chunks_.end(); ++it) {
    const Chunk& chunk = *it;
    if (chunk.HasBlock(p, chunkLength)) return &chunk;
  }
  return NULL;
}
...
bool FixedAllocator::TrimEmptyChunk(void)
{
  ...
  if (lastChunk != emptyChunk_) std::swap(*emptyChunk_, *lastChunk);
...
bool FixedAllocator::TrimChunkList(void)
{
  ...
  if (chunks_.size() == chunks_.capacity()) return false;
...
bool FixedAllocator::MakeNewChunk(void)
{
  ...
  try {
    ...
    if (allocated) chunks_.push_back(newChunk);
  } catch (...) {
    allocated = false;
  }
...
bool FixedAllocator::Deallocate(void* p, Chunk* hint)
{
  ...
  if (NULL == foundChunk) return false;
...
void FixedAllocator::DoDeallocate(void* p)
{
  ...
  if (deallocChunk_->HasAvailable(numBlocks_)) {
    ...
    if (NULL != emptyChunk_) {
      ...
      lastChunk->Release();
      chunks_.pop_back();
      if ((allocChunk_ == lastChunk) || allocChunk_->IsFilled()) allocChunk_ = deallocChunk_;
    }
...
void* DefaultAllocator(std::size_t numBytes, bool doThrow)
{
  ...
  if (doThrow && (NULL == p)) throw std::bad_alloc();
  return p;
#endif
}
...
SmallObjAllocator::SmallObjAllocator(std::size_t pageSize, std::size_t maxObjectSize, std::size_t objectAlignSize)
    : pool_(NULL), maxSmallObjectSize_(maxObjectSize), objectAlignSize_(objectAlignSize)
{
#ifdef DO_EXTRA_LOKI_TESTS
  ...
  for (std::size_t i = 0; i < allocCount; ++i) pool_[i].Initialize((i + 1) * objectAlignSize, pageSize);
}
...
bool SmallObjAllocator::TrimExcessMemory(void)
{
  ...
  std::size_t       i          = 0;
  for (; i < allocCount; ++i) {
    if (pool_[i].TrimEmptyChunk()) found = true;
  }
  for (i = 0; i < allocCount; ++i) {
    if (pool_[i].TrimChunkList()) found = true;
  }
...
{
  if (numBytes > GetMaxObjectSize()) return DefaultAllocator(numBytes, doThrow);
  ...
  if ((NULL == place) && TrimExcessMemory()) place = allocator.Allocate();
...
bool SmallObjAllocator::IsCorrupt(void) const
{
  ...
  const std::size_t allocCount = GetOffset(GetMaxObjectSize(), GetAlignment());
  for (std::size_t ii = 0; ii < allocCount; ++ii) {
    if (pool_[ii].IsCorrupt()) return true;
  }
  return false;
}
