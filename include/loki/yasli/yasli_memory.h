...
struct memmove_traits
{
  ...
  static T *nondestructive_assign_move(
    ...
    T *dest)
  {
    yasli_nstd::destroy_range(begin, end);//error
    memmove(dest, begin, (end - begin) * sizeof(T));
    return static_cast<T *>(dest) + (end - begin);
  }
};
...
template<class T>
struct move_traits : public yasli_nstd::type_selector<yasli_nstd::is_class<T>::value == 0, memmove_traits<T>, safe_move_traits<T>>::result
{
};

template<class T>
struct move_traits<std::complex<T>> : public yasli_nstd::type_selector<sizeof(std::complex<T>) == 2 * sizeof(T), memmove_traits<std::complex<T>>, safe_move_traits<std::complex<T>>>::result
{
};
}// namespace yasli_protocols
namespace yasli_nstd {
...
void destroy(A &a, T *p, typename A::size_type n)
{
  yasli_nstd::type_selector<yasli_nstd::is_class<T>::value != 0,
    _impl::destroyer,
    _impl::non_destroyer>::result::destroy(a, p, n);
}
void bar() {}
...
void destroy_range(ForwardIterator b, ForwardIterator e)
{
  yasli_nstd::type_selector<
    yasli_nstd::is_class<typename std::iterator_traits<ForwardIterator>::value_type>::value != 0,
    _impl::destroyer,
    _impl::non_destroyer>::result::destroy_range(b, e);
}
  ...
  : public generic_allocator_traits<yasli::allocator<T>>
{
  ...
  static bool reallocate_inplace(
    ...
    typename A::size_type newSize)
  {
    allocator_traits<yasli_nstd::mallocator<T>>::reallocate_inplace(a, b, newSize);
  }
  ...
  {
    allocator_traits<yasli_nstd::mallocator<T>>::reallocate(a, b, e, newSize);
  }
...
T *uninitialized_copy(const T *first, const T *last, T *result)
{
  ...
  return yasli_nstd::type_selector<yasli_nstd::is_memcopyable<T>::value != 0,
    ...
    _impl::uninitialized_safe_copier<const T *, T *>>::result::execute(first, last, result);
}
...
template<class T, class U>
void uninitialized_fill(T *first, T *last, const U &x)
{
  struct ScopeGuard
  {
    T *first;
