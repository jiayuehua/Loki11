...
namespace Private {
  ...
  public:
    inline explicit TwoRefCountInfo(bool strong)
      : m_pointer(0), m_strongCount(strong ? 1 : 0), m_weakCount(strong ? 0 : 1)
    {
    }

    inline TwoRefCountInfo(void *p, bool strong)
      : m_pointer(p), m_strongCount(strong ? 1 : 0), m_weakCount(strong ? 0 : 1)
    {
    }
  ...
  public:
    inline explicit LockableTwoRefCountInfo(bool strong)
      : TwoRefCountInfo(strong), m_Mutex()
    {
    }

    LockableTwoRefCountInfo(void *p, bool strong)
      : TwoRefCountInfo(p, strong), m_Mutex()
    {
    }
...
protected:
  ...
  TwoRefCounts(const TwoRefCounts &rhs, bool strong) : m_counts(rhs.m_counts)
  {
    Increment(strong);
  }
...
protected:
  ...
  LockableTwoRefCounts(const LockableTwoRefCounts &rhs, bool strong) : m_counts(rhs.m_counts)
  {
    Increment(strong);
  }
...
private:
  ...
  typedef typename std::conditional<CP::allow, Tester, unspecified_boolean_type_>::type
...
private:
  ...
  typedef typename std::conditional<CP::allow, PointerType, Insipid>::type
...
template<
  ...
  class CNP>
inline bool operator==(
  const StrongPtr<T, S, OP, CP, KP, RP, DP, CNP> &lhs,
  U *rhs)
{
